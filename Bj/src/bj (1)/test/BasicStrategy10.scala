...
class BasicStrategy10 extends Logs {
 ...
 @Test
  def test {
	...
	// Create a player with a bank roll of $100 and betting $25
    val player = new BetterPlayer("Kris", 100, 25)
    ...
    // Deal two cards
    player ! Card(8,Card.CLUB)
    player ! Card(8,Card.DIAMOND)
    
    // Send the up-card to tell player it's its turn
    player ! Up(Card(5,Card.SPADE))
    
     // Wait for the player to respond
    receiveWithin(2000) {
      case Split(player.pid) =>
        debug("TEST PASSED Split Request received")
        assert(true)
